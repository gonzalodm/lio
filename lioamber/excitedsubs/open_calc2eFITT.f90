...
use faint_cpu, only: int3lu
   ...
   real*8, intent(in) :: Mata(M,M,numvec), Matb(M,M,numvec)
   ...
   MM = M*(M+1)/2
   allocate(Vec(MM),Fock_vec(MM),Tmat(M,M))
...
!  Pointers to Gmat and Ginv
   ...
   do ist=1,numvec
      Tmat = Mata(:,:,ist) + Matb(:,:,ist)
      do i=1,M
      do j=1,i-1
         Tmat(i,j) = Tmat(i,j) + Tmat(j,i)
      enddo
      enddo
      call sprepack('L',M,Vec,Tmat)
...
!     Transform vec -> mat
   ...
   deallocate(Fmat_b,Hmat,Vec,Fock_vec,Tmat)
