...
use garcha_mod, only: natom, Md, RMM
   ...
   MM = M*(M+1)/2 
   MMd = Md*(Md+1)/2
   M9 = 1+3*MM+MMd ! pointer to Ginv

   ! Put Densities in vector form (triang superior requiere)
   ...
   do i=1,M
      ...
      do j=i+1,M
         ...
         !print*, ind,E_vec(ind)
         !print*,i,j,ind,rhoExc(i,j),rhoExc(j,i),factor
         !print*,i,j,ind,rhoDif(i,j),rhoDif(j,i)
         ind = ind + 1
      enddo
   enddo

   ! Calc Variational coef with differents densities
   ...
   print*,"forces (P|Q) coulomb"
   do i=1,natom
      print*,i,for(i,1),for(i,2),for(i,3)
   enddo
   !do i=1,Md
   !   print*, i,af_Exc(i)
   !enddo
...
implicit none
   ...
   real*8, intent(inout) :: afE(Md), afD(Md), afX(Md)
   ...
   do kk_ind = 1, kknumd
      ...
      do k_ind = 1, Md
         ...
         ind = kkind(kk_ind)
         RcE(k_ind) = RcE(k_ind) + Evec(ind) * cool(iikk+k_ind)
         RcD(k_ind) = RcD(k_ind) + Dvec(ind) * int3d
         RcX(k_ind) = RcX(k_ind) + Xvec(ind) * int3d
         !print*, "coolEXC dou", cool(iikk+k_ind), Evec(ind)
      enddo
   enddo
   ...
   do kk_ind = 1, kknums
      ...
      do k_ind = 1, Md
         ...
         ind = kkinds(kk_ind)
         RcE(k_ind) = RcE(k_ind) + Evec(ind) * cools(iikk+k_ind)
         RcD(k_ind) = RcD(k_ind) + Dvec(ind) * int3s
         RcX(k_ind) = RcX(k_ind) + Xvec(ind) * int3s
         !print*, "coolEXC sim", cools(iikk+k_ind), Evec(ind)
      enddo
   enddo
...
use liosubs_math , only: FUNCT
use garcha_mod   , only: a, c, nuc, nucd, ad, cd, ncont, ncontd, nshell,&
                         nshelld, M, NORM, rmax, r, d
